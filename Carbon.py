...
class window(QWidget):
    ...
    # -- FORMAT SELECTOR --
        ...
        self.format_combo.setFixedWidth(52)
    ...
    # -- OUTPUT + ERROR TABS --
        ...
        self.error_console.setReadOnly(True)
            ...
            # Attaches them to tabs with names
    ...
    def change_format(self, text):
        self.selected_format = text.lower()
        self.update_command()

    def update_command(self):
        link = f" {self.link_input.text()}"
        filename = self.filename_input.text()
        formatt = ""

            # If the filename input isn't blank: use the -o command with the name in the input box, if not: keep it blank
        ...
        if self.selected_format == "Auto":
            fileformat = ""
            # Else if mp4 format is selected: insert mp4 command
        elif self.selected_format == "mp4":
            # downloads the best video and audio available and merges then into the selected format
            fileformat = f""" -f "bestvideo+bestaudio" --merge-output-format {self.selected_format}"""
            # Else if mp4 format is selected: insert mp3 command
            ...
            fileformat = f" -x --audio-format {self.selected_format}"

            # Anatomy of the yt-dlp input command
        command = f"yt-dlp{link}{filename}{fileformat}"
